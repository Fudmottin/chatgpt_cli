...
void set_command(OpenAIClient& ai_client, const vector<string>& parts,
   ...
   try {
      ...
      cout << "Invalid " << parameter_name
           << " value. Please provide a valid number." << endl;
   } catch (const std::out_of_range& e) {
      cout << parameter_name
           << " value out of range. Please provide a valid number within the "
              "acceptable range."
           << endl;
   }
}

void set_temperature_command(OpenAIClient& ai_client,
                             const vector<string>& parts) {
   set_command(ai_client, parts, &OpenAIClient::set_temperature, "temperature");
}

void set_presence_penalty_command(OpenAIClient& ai_client,
                                  const vector<string>& parts) {
   set_command(ai_client, parts, &OpenAIClient::set_presence_penalty,
               "presence penalty");
}

void set_frequency_penalty_command(OpenAIClient& ai_client,
                                   const vector<string>& parts) {
   set_command(ai_client, parts, &OpenAIClient::set_frequency_penalty,
               "frequency penalty");
}

void quit_command(OpenAIClient& ai_client, const vector<string>& parts) {
   cout << "Quitting program." << endl;
   string clanker_dir = util::get_clanker_dir();
   if (clanker_dir != "") {
      string time_stamp = util::get_formatted_time();
      string history_file =
         clanker_dir + "/chatgpt_history_" + time_stamp + ".txt";
      if (ai_client.save_history(history_file))
...
void prompt_image_command(OpenAIClient& ai_client,
                          const vector<string>& parts) {
   if (parts.size() < 2) {
   ...
   image_prompt +=
      " Create a prompt for DALL-E that is fewer than 1000 characters.";
...
void handle_command(const string& command, OpenAIClient& ai_client) {
   ...
   transform(trimmed_command.begin(), trimmed_command.end(),
             trimmed_command.begin(),
...
int main(int argc, char* argv[]) {
   ...
   util::history_filename = util::get_clanker_dir() + "/history";
