...
string get_api_key() {
   ...
   if (api_key.empty()) {
      cerr << "Failed to read the API key from the .env file. Please ensure it "
              "is properly set."
           << endl;
      return "";
   }
...
string get_clanker_dir() {
   string home_dir = get_home_directory();
   string clanker_dir = home_dir.empty() ? "" : home_dir + "/.config/clanker";
   if (!clanker_dir.empty()) {
      struct stat st;
      if (stat(clanker_dir.c_str(), &st) == -1) {
         if (mkdir(clanker_dir.c_str(), 0700) == -1) {
