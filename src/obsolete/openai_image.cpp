...
void OpenAIImage::set_image_size(const string& size) { image_size = size; }
...
string OpenAIImage::send_message(const string& message) {
   ...
   if (response.status_code == 200) {
      ...
      cerr << "Error: Request failed with status code " << response.status_code
           << endl;
...
string OpenAIImage::process_result(const json& result) {
   ...
   string save_dir = util::get_clanker_dir();
   ...
   if (result.contains("data")) {
      for (const auto& entry : result["data"]) {
         if (entry.contains("b64_json")) {
            ...
            auto binary_data =
               cppcodec::base64_rfc4648::decode(image_data_base64);

            string file_name = save_dir + "/image_" +
                               util::get_formatted_time() + "_" +
                               to_string(counter) + ".png";
            ofstream image_file(file_name, ios::binary);
            image_file.write(reinterpret_cast<const char*>(binary_data.data()),
                             binary_data.size());
            image_file.close();
...
string OpenAIImage::join(const vector<string>& strings,
