...
string ChatClient::send_message(const string& message) {
   ...
   try {
      ...
      cout << "Unknown exception thrown from ChatClient::send_request()"
           << endl;
      return string("\n");
   }
...
json ChatClient::send_request(const json& request_data) {
   ...
   updated_request_data["messages"] =
      json::array({{{"role", "user"}, {"content", concatenated_context}}});
   ...
   if (response.status_code != 200) {
      cerr << "Error: " << response.status_code << " " << response.error.message
           << endl;
...
string ChatClient::extract_response(const json& response) {
   if (response.contains("choices") && !response["choices"].empty()) {
      const auto& message =
         response["choices"][0]["message"]["content"].get<string>();
      return message;
   } else {
...
bool ChatClient::save_history(const string& filename) {
   ...
   try {
      for (auto line : conversation_history) ofs << line;
      ofs.close();
